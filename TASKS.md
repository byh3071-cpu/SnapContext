...
- [ ] **P0 — 0.4.3 마무리 (사람 게이트)**
  - 구현·critic 적대검증 완료(2026-08-16, 커밋 835dca0·142 tests green) — 주석 4종(가리기=솔리드 박스 파괴적 bake)·토큰 재발급 lite. 상세: docs/log/2026-08-16-0.4.3-annotation-redaction.md.
  - 남은 것: 수동 QA(로그의 5항목 체크리스트) → tag `v0.4.3`. 스토어 제출 없음.

- [ ] **P1 — 스토어 재심사 제출: 0.4.5 개발 완료 후 마지막에 1회 (요한 결정 2026-08-16)**
  - 0.4.2~0.4.5를 제출 없이 개발 진행 → 0.4.5 완성본으로 재심사 #1 제출(중간 버전마다 심사 대기 반복 안 함).
  - 0.4.4(ADR-015 2차 배포)·0.4.5(CF native RL) 스코프는 착수 시 정의(로드맵 트랙 F).
