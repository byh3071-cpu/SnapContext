...
- [ ] **P0 — 0.4.4 스코프 정의 (다음 착수점)**
  - ADR-015 2차 배포(worker-only): `/s` 제거·무서명 `/i` 403·`{id,url}`→`{id}` 일괄. 착수 조건이던 "신확장 롤아웃 충분"은 유저 ~0이라 요한 로컬 설치로 대체 판정(로드맵 트랙 F).
  - ⚠️ **감사 핸드오프 필독(2026-08-17)**: `docs/state/handoff-audit-0.4.4.md` — H1(레거시 캡처 삭제가 R2 안 지움, 검증됨)이 PRD-0.4.4 **D2 잔존 실측 게이트와 직결**(revert 경로 시 삭제 핸들러 수정 필수). 편승 후보 7건(masthead·qa-043 게이트 등)은 요한 승인 후 편입. 전문: `docs/research/predeploy-audit-0.4.x-2026-08-17.md`.
