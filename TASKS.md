...
- [ ] **P0 — 0.4.4 배포 완료·마감 3건 잔여 (2026-08-17 배포 실행됨)**
  ...
  - **배포 완료**: 잔존 실측(R2 레거시 키 0·lifecycle 전prefix 30일·D1 레거시 1행 삭제 완료) → `pnpm run deploy` 성공(Version `2feb2238`) → **스모크 7항목 전부 PASS**(2026-08-17 23시대, 삭제 1클릭만 잔여). 실행 기록: docs/runbook-0.4.4.md §6.
  - 스모크 중 발견·해결 2건(둘 다 troubleshooting 문서화): dist 로컬 endpoint 잔존(재빌드)·프로덕션 D1 0002 미적용 500(apply). 교훈: 배포 런북에 `d1 migrations list --remote` 확인 단계 고정.
  - 남은 것: 확장에서 방금 캡처 삭제→목록 소멸 1클릭 확인 → tag `v0.4.4`.
