...
describe('V3 identity fields (진단용 argv)', () => {
  it('다른 port/IP 명령은 identity 정규화에서 거부한다', async () => {
    const { normalizeCommandIdentity } = await loadLib()
    expect(() =>
      normalizeCommandIdentity(OTHER_PORT_CMD, {
