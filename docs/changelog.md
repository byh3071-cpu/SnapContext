...
## 0.4.6 — tag v0.4.6 (2026-08-30) · 스토어 일괄 재심사 제출 대기 (ext-only)
...
- 가리기는 모자이크·블러를 제공하지 않는다 — 픽셀레이션 텍스트는 Depix·Unredacter로 복원되기 때문(솔리드 채움만 복원 불성립).
- 설정에 `토큰 재발급` 버튼을 추가했다(ADR-020 Tier 1). 이전 토큰은 기존 캡처 만료(≤30일)까지 유효함을 고지한다. 완전 revoke는 0.4.7(ADR-022로 이동, 2026-08-30 정정).
- 타입: `Annotation` 유니언(RedactBox·Arrow·Stroke) 신설, `CaptureHistoryItem.hasAnnotations` 추가. 주석은 세션-로컬(히스토리 복원 시 미복원 — 조건부 고지).
- worker 무변경 — serverInfo는 0.4.2 유지(ADR-014 2트랙).
