...
| # | 항목 | 확인 방법 |
|---|---|---|
| 1 | 확장 코드 = tag `v0.4.6`(코드 동결) | `git diff v0.4.6 --stat -- src manifest.json package.json` → 변경 0(태그 뒤 커밋은 문서·스크립트만 허용) |
...
| 4 | 스토어 등록 이미지 5장이 현행 UI(요약 카드·버그/참고 토글·"내 AI에 저장"·저장 완료 안내)인지 눈으로 확인 | `docs/store/chrome-web-store/screenshots/*.png` |
| 5 | 권한 변화 없음 — **확인됨(2026-08-30)**: `git show v0.3.0:manifest.json`과 현재 manifest의 permissions(sidePanel·storage·scripting·downloads·tabs·windows)·host `<all_urls>`·commands 5개 완전 동일 | Privacy practices 재답변 불필요 |
...
```powershell
# 저장소 루트, PowerShell
pnpm build                          # 프로덕션 endpoint · version-sync 0.4.6 확인
python scripts/pack-store-zip.py    # snapcontext-v0.4.6.zip — '/' 경로 강제(PAT-001), 로컬 endpoint 빌드면 중단
```

> ⚠️ `dogfood:up`을 돌린 뒤라면 dist가 **로컬 endpoint(127.0.0.1:8787) 빌드**다. 스크립트가 감지해 중단하니 `pnpm build`를 다시 돌린 뒤 실행.
...
3. 등록 이미지 5장 교체(01~05, 1280×800) — 기존 04 "공유 링크" 이미지는 반드시 삭제
...
3. 등록 이미지 5장 교체 · 분류 생산성 · 성인콘텐츠 아니요 → 제출
