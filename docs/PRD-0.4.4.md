...
| D2 | ADR-015 "30일 대기" 대체 | **배포 직전**(사람 게이트) production R2의 레거시(raw-ID key) 객체 잔존 실측 — 0이면 일괄 배포, 잔존 시 스코프 #7 커밋만 revert한 빌드로 배포. 실측 수단은 wrangler R2 목록 또는 대시보드(게이트 시점 확인) |
...
- **롤백 런북**: 구버전 Worker 롤백 **금지**(공개 경로 재개방 = 개인정보 회귀) → 410 forward-fix 우선. 사람 게이트.
