...
- [2026-07-22] base64url 로 16바이트를 인코딩하면 22자(132비트)라 마지막 문자의 하위 4비트가 미사용 → 같은 바이트열에 유효한 문자열 표현이 16개 존재. 서명 검증은 디코드한 바이트로 하면서 식별자는 문자열 전체를 해시하면, 검증은 통과하는데 식별자만 갈라진다 (PAT-002).
- [2026-08-05] 공개 R2 키를 제자리에서 비공개로 바꾸면 rollback 때 다시 노출될 수 있다. private-v2 키 공간과 신규 `/captures`·`/pi` 경로를 분리해야 구버전 rollback도 새 객체를 공개하지 못한다.
- 2026-08-30: 게이트(단위·tsc·build·goal 게이트) 전부 초록이어도 다른 벤더 적대 검증은 매 웨이브 결함을 냈다(0.4.6 세 웨이브 15건). 게이트 초록 = "안 깨졌다"이지 "맞다"가 아니다. 용어 일괄 치환은 인용 문서부터 제외(PAT-004), 런타임 단언은 성공 경로 밖으로(PAT-005).
